...
export default function ClientsPage() {
  ...
  const filtered = mockActusClients.filter(c =>
    c.nome_razao_social.toLowerCase().includes(search.toLowerCase()) ||
    (c.cpf_cnpj || '').includes(search)
  );

  const pf = mockActusClients.filter(c => c.tipo_pessoa === 'PF').length;
  const pj = mockActusClients.filter(c => c.tipo_pessoa === 'PJ').length;
        ...
        <motion.div initial="hidden" animate="visible" variants={{ hidden: {}, visible: { transition: { staggerChildren: 0.05 } } }}>
          <motion.div variants={fadeUp} className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-sm text-muted-foreground">{mockActusClients.length} clientes cadastrados</p>
            </div>
          ...
          <motion.div variants={fadeUp} className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
            <KpiCard title="Total de Clientes" value={mockActusClients.length} icon={<Users className="h-5 w-5" />} />
            ...
            <KpiCard title="Contratos Vigentes" value={mockActusClients.filter(c => c.status_cliente === 'Ativo').length} icon={<Users className="h-5 w-5" />} variant="success" />
          </motion.div>
          ...
          <motion.div variants={fadeUp} className="bg-card rounded-lg border border-border shadow-card overflow-hidden">
            ...
            <div className="border-t border-border px-4 py-3">
              <span className="text-xs text-muted-foreground">Mostrando {filtered.length} de {mockActusClients.length} clientes</span>
            </div>
          </motion.div>
        </motion.div>
      </div>
